...
Status: pre-analysis catalogue and mock validation, 2026-09-24. We have parsed all eight public real-data and random FITS headers, inspected the four full data catalogues and all four full random catalogues for selection metadata, audited 1000 matched EZmock filename sets, checked 24 bounded realistic-mock FITS headers, and inspected 12 full realistic-mock data files for selection metadata only. The exact common mask, full mock selection and cross-covariance remain unvalidated. No eBOSS odd-sector vector has been calculated or inspected, and the analysis protocol is not yet frozen.
