#!/usr/bin/env python3
"""Select the strongest hidden-state candidate within one control sweep."""
...
from class_scalar_control_run import control_parameters
...
def main() -> None:
    ...
    ap.add_argument("--control", required=True,
                    choices=["fixed-total-matter", "fixed-relic-rho-match"])
    ...
    for tag, mass in MASS_GRID:
        ...
        cp = control_parameters(args.control, mass, args.z_match)
        rows.append({
            "tag": tag,
            "mass_eV": mass,
            "winner": winner,
            ...
            "natural_omega_ncdm_deg1": cp["natural_omega_ncdm_deg1"],
        })
    ...
    winner_params = control_parameters(
        args.control, float(best["mass_eV"]), args.z_match
    )
