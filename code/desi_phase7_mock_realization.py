#!/usr/bin/env python3
"""One cut-sky mock realization for the Phase-7 five-tracer estimator.

The mock is split into five luminosity-ranked tracers in the same narrow-z,
NGC/SGC structure as the real-data analysis.  The clustering-ready mock odd
vector is measured with the same inverse-probability pair compression.

The wake and Doppler shapes are then forward-projected through the *actual
sampled pair geometry* of the realization.  For a unit tracer-antisymmetric
pair signal

    xi_odd^{ij}(s,mu) = (m_j-m_i) mu T(s,z),

the code evaluates the corresponding response of the same marked dipole
estimator.  This calibrates survey geometry, angular cuts, finite separation
bins and the proxy-mark compression.  It remains a luminosity-proxy response;
conversion to a physical halo-mass wake amplitude is a separate step.
"""
...
def read_mock(paths, kind):
    ...
    for path in paths:
        ...
        with fits.open(path, memmap=True) as hdul:
            ...
            if kind=='data':
                names={n.upper():n for n in d.names}
                if 'R_MAG_APP' in names:
                    mag=np.asarray(d[names['R_MAG_APP']],float); field='R_MAG_APP'
                elif 'R_MAG_ABS' in names:
                    mag=np.asarray(d[names['R_MAG_ABS']],float); field='R_MAG_ABS'
                else:
                    raise KeyError('BGS mock needs R_MAG_APP or R_MAG_ABS for the predeclared luminosity proxy')
                if rank_field is None: rank_field=field
                elif rank_field != field: raise RuntimeError('inconsistent mock luminosity fields across caps')
                # mt.make_data_proxy only needs a monotonic flux representation.
                flux=np.power(10.0, np.clip((22.5-mag)/2.5, -100, 100))
            else:
    ...
    return cat, rank_field


def forward_block(block, template_path, z_eff):
    """Response to unit antisymmetric pair template through frozen DD geometry."""
    ...
    # estimator factor (3 mu dm) times injected odd pair probability (mu dm T)
...
def main():
    ...
    catD,rank_field=read_mock(args.data,'data'); catR,_=read_mock(args.random,'random')
    ...
    # No delete-one covariance is needed per realization; zero labels are sufficient for the shared estimator helpers.
    ...
    for iz,(lo,hi) in enumerate(zip(zedges[:-1],zedges[1:])):
        ...
        DD=mt.sample_pairs(D,D,edges,args.neighbors_per_anchor,rng,args.theta_min_deg,auto=True,label=f'M{mid}_DD_z{iz}')
        DR=mt.sample_pairs(D,R,edges,args.neighbors_per_anchor,rng,args.theta_min_deg,auto=False,label=f'M{mid}_DR_z{iz}')
        RR=mt.sample_pairs(R,R,edges,args.neighbors_per_anchor,rng,args.theta_min_deg,auto=True,label=f'M{mid}_RR_z{iz}')
    ...
    np.savetxt(out/f'{stem}_vector.csv',np.asarray(rows),delimiter=',',
               header='zlo,zhi,z_effective,s_Mpc_over_h,xi0_proxy,xi1_proxy_odd',comments='')
    np.savetxt(out/f'{stem}_window.csv',np.column_stack([wake,dop]),delimiter=',',
               header='wake_forward_response,doppler_forward_response',comments='')
    summary={
      'mock_id':mid,'scope':'cut-sky BGS mock five-tracer luminosity-ranked proxy validation',
      'luminosity_rank_field':rank_field,'data_count':int(len(iD)),'random_count':int(len(iR)),
      'ntracer':int(args.ntracer),'z_bins':zmeta,'separation_edges_Mpc_over_h':edges.tolist(),
      'neighbors_per_anchor':int(args.neighbors_per_anchor),'theta_min_deg':args.theta_min_deg,
