#!/usr/bin/env python3
"""Select the strongest candidate in the fixed-omega0 + fixed-early-Neff sweep."""
...
from class_scalar_fixed_omega0_neff_run import CONTROL, control_parameters
...
def main() -> None:
    ...
    for tag, mass in MASS_GRID:
        d = args.root / tag
        p = d / "scalar_candidate_actual_summary.json"
        if not p.exists():
        ...
        rows.append({
            ...
            "mass_eV": mass,
            "winner": winner,
            "winner_strength_ideal_SN": float(s["winner_strength_ideal_SN"]),
            ...
            "controlled_early_Neff": cp["controlled_early_Neff"],
        })

    best = max(rows, key=lambda r: r["winner_strength_ideal_SN"])
    src = args.root / best["tag"]
    args.outdir.mkdir(parents=True, exist_ok=True)
    for name in ("winner_plus.dat", "winner_minus.dat", "scalar_candidate_actual_summary.json"):
        shutil.copy2(src / name, args.outdir / name)
    ...
    out = {
        ...
        "ranking_metric": (
            "max(ideal full-sky joint TT+TE+EE S/N, ideal full-sky phi-phi auto S/N)"
        ),
        "per_mass": rows,
        "global_winner": best,
