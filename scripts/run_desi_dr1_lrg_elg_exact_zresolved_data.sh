...
python code/desi_dr1_lrg_elg_exact_zresolved.py \
  --lrg-data "${ROOT}/LRG_NGC_clustering.dat.fits" "${ROOT}/LRG_SGC_clustering.dat.fits" \
  --elg-data "${ROOT}/ELG_LOPnotqso_NGC_clustering.dat.fits" "${ROOT}/ELG_LOPnotqso_SGC_clustering.dat.fits" \
  --lrg-random "${lr[@]}" \
  --elg-random "${er[@]}" \
  --outdir "$OUT" \
